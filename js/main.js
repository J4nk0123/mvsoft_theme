...
// AI Assistant
    ...
    if ($('.section-tourist-guides__img').length) {
      ...
      const hideElements = (pinNumber) =>
        $(`.section-tourist-guides__img-${pinNumber}, .section-tourist-guides__popup-${pinNumber}`).css({ opacity: 0, pointerEvents: 'none' });

      const showElements = (pinNumber) => {
        $(`.section-tourist-guides__img-${pinNumber}, .section-tourist-guides__popup-${pinNumber}`).css({ opacity: 1, pointerEvents: 'auto', zIndex: 1 });
      };
